...
async function startThakku() {
    ...
    store.bind(Thakku.ev)
    
    // anticall auto block
    Thakku.ws.on('CB:call', async (json) => {
    ...
    Thakku.ev.on('messages.upsert', async chatUpdate => {
    ...
    // Group Update
    Thakku.ev.on('groups.update', async pea => {
       //console.log(pea)
    // Get Profile Picture Group
       try {
       ppgc = await Thakku.profilePictureUrl(pea[0].id, 'image')
       } catch {
       ppgc = 'https://shortlink.Thakkuarridho.my.id/rg1oT'
       }
    ...
    Thakku.ev.on('group-participants.update', async (anu) => {
        console.log(anu)
        try {
            let metadata = await Thakku.groupMetadata(anu.id)
            let participants = anu.participants
            for (let num of participants) {
                // Get Profile Picture User
                try {
                    ppuser = await Thakku.profilePictureUrl(num, 'image')
                } catch {
    ...
    Thakku.ev.on('contacts.update', update => {
        for (let contact of update) {
            let id = Thakku.decodeJid(contact.id)
            if (store && store.contacts) store.contacts[id] = { id, name: contact.notify }
        }
    })

    Thakku.getName = (jid, withoutContact  = false) => {
        id = Thakku.decodeJid(jid)
        withoutContact = Thakku.withoutContact || withoutContact 
        let v
        if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
            v = store.contacts[id] || {}
            if (!(v.name || v.subject)) v = Thakku.groupMetadata(id) || {}
            resolve(v.name || v.subject || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
        })
        else v = id === '0@s.whatsapp.net' ? {
            id,
            name: 'WhatsApp'
        } : id === Thakku.decodeJid(Thakku.user.id) ?
            Thakku.user :
            (store.contacts[id] || {})
            return (withoutContact ? '' : v.name) || v.subject || v.verifiedName || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international')
    }
    
    Thakku.sendContact = async (jid, kon, quoted = '', opts = {}) => {
	let list = []
	for (let i of kon) {
	    list.push({
    ...
    Thakku.serializeM = (m) => smsg(Thakku, m, store)

    Thakku.ev.on('connection.update', async (update) => {
     ...
     * @param {*} options 
     */
    Thakku.sendButtonText = (jid, buttons = [], text, footer, quoted = '', options = {}) => {
        let buttonMessage = {
            text,
            footer,
            buttons,
        ...
        Thakku.sendMessage(jid, buttonMessage, { quoted, ...options })
    }
     ...
     * @returns 
     */
    Thakku.sendText = (jid, text, quoted = '', options) => Thakku.sendMessage(jid, { text: text, ...options }, { quoted })
     ...
     * @returns 
     */
    Thakku.sendImage = async (jid, path, caption = '', quoted = '', options) => {
	let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await Thakku.sendMessage(jid, { image: buffer, caption: caption, ...options }, { quoted })
    }
     ...
     * @returns 
     */
    Thakku.sendVideo = async (jid, path, caption = '', quoted = '', gif = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await Thakku.sendMessage(jid, { video: buffer, caption: caption, gifPlayback: gif, ...options }, { quoted })
    }
     ...
     * @returns 
     */
    Thakku.sendAudio = async (jid, path, quoted = '', ptt = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await Thakku.sendMessage(jid, { audio: buffer, ptt: ptt, ...options }, { quoted })
    }
     ...
     * @returns 
     */
    Thakku.sendTextWithMentions = async (jid, text, quoted, options = {}) => Thakku.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
     ...
     * @returns 
     */
    Thakku.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
        ...
        await Thakku.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
        return buffer
    }
     ...
     * @returns 
     */
    Thakku.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
        let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        let buffer
        if (options && (options.packname || options.author)) {
     ...
     * @returns 
     */
    Thakku.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
    ...
    Thakku.downloadMediaMessage = async (message) => {
        let mime = (message.msg || message).mimetype || ''
        let messageType = message.mtype ? message.mtype.replace(/Message/gi, '') : mime.split('/')[0]
     ...
     * @returns 
     */
    Thakku.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
        let types = await Thakku.getFile(path, true)
       ...
       await Thakku.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options })
     ...
     * @returns 
     */
    Thakku.copyNForward = async (jid, message, forceForward = false, options = {}) => {
        let vtype
        ...
        await Thakku.relayMessage(jid, waMessage.message, { messageId:  waMessage.key.id })
        return waMessage
    }

    Thakku.cMod = (jid, copy, text = '', sender = Thakku.user.id, options = {}) => {
		...
		copy.key.fromMe = sender === Thakku.user.id
     ...
     * @returns 
     */
    Thakku.getFile = async (PATH, save) => {
        let res
